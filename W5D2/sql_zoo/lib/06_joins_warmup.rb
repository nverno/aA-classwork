...
def glenn_close_id
  ...
  execute(<<-SQL)
  SELECT id 
  FROM actors WHERE name = 'Glenn Close'
  SQL
end
...
def casablanca_cast
  ...
  execute(<<-SQL)
  SELECT name FROM actors WHERE id IN 
  (SELECT actor_id FROM castings
    WHERE movie_id =
    (SELECT id FROM movies WHERE title = 'Casablanca'))
  SQL
end
...
def alien_cast
  ...
  WHERE movie_id = (SELECT id FROM movies WHERE title = 'Alien')
  SQL
end
